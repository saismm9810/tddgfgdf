...
const GET_COINMARKETCAP_DATA = "GET_COINMARKETCAP_DATA";
...
export default function Provider({ children }) {
  ...
  const getCoinMarketCapData = useCallback((data) => {
    dispatch({
      type: GET_COINMARKETCAP_DATA,
      payload: { data },
    });
  }, []);
  return (
...
const getCoinMarketCapDataFunc = async () => {
  let data = [];

  let qs = `?start=1&limit=3&convert=USD`;
  try {
    let res = await axios.get(
      ...
      {
        headers: {
          "X-CMC_PRO_API_KEY": "876f4f9d-bc9b-43be-a12e-a702877182dc",
...
export function useGetCoinMarketCapData() {
  const [state] = useTokenDataContext();

  let data = getCoinMarketCapDataFunc();
}
