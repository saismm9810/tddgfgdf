...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://pancakeswap.finance/swap?inputCurrency=${token0Address}`
  } else {
    return `https://pancakeswap.finance/swap?inputCurrency=${
      token0Address === '0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c' ? 'BNB' : token0Address
...
export function getMiningPoolLink(token0Address) {
  return `https://pancakeswap.finance/uni/ETH/${token0Address}`
}

export function getUniswapAppLink(linkVariable) {
  let baseUniswapUrl = 'https://pancakeswap.finance/uni'
  if (!linkVariable) {
